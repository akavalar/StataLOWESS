...
program define ksm_original7, sort
	version 7
...
*/
			local smopt = ("`line'"=="line") /* 
				*/ + cond("`weight'"=="weight",2,0)
...
*/
	...
	gen double `W' = 1 in 1/`count'
	gen double `Ys' = .
	local wtd = (`smopt'>1)
	local mean = (`smopt'==0 | `smopt'==2)
	local k = int((`bwidth'*`count'-0.5)/2)
	local i 0
	while `i'<`count' {
		local i = `i'+1
		local x0 = `X'[`i']
		local imk = max(1, `i'-`k')
		local ipk = min(`i'+`k', `count')
		if `wtd' {
			local delta = 1.0001* /*
				*/ max(`X'[`ipk']-`x0', `x0'-`X'[`imk'])
			replace `W' = cond(`delta'>1e-30, /*
				*/ (1-(abs(`X'-`x0')/`delta')^3)^3 , 1) /*
				*/ in `imk'/`ipk'
		}
		if `mean' {
			summ `Y' [aw=`W'] in `imk'/`ipk'
			replace `Ys' = r(mean) in `i'
		}
		else {
			reg `Y' `X' [aw=`W'] in `imk'/`ipk'
